...
select c2.name, r.duration, r.distance
 from cities_routes r inner join cities c1 on r.from = c1.id inner join cities c2 on r.to = c2.id where c1.name = 'Warszawa'
 order by duration limit 10;
...
select sum(population) as total from cities c inner join cities_routes r on r.to = c.id group by r.from limit 1;
